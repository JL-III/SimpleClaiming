...
import me.ryanhamshire.GriefPrevention.visualization.VisualizationDep;
...
public class VisualizationDep {
     ...
     */
    @Deprecated(forRemoval = true, since = "16.18")
    public static Visualization FromClaim(Claim claim, int height, VisualizationTypeDep visualizationType, Location locality)
    {
        ...
        visualization.boundaries.addAll(
                claim.children.stream()
                        .map(child -> new Boundary(child, SUBDIVISION))
     ...
     */
    @Deprecated(forRemoval = true, since = "16.18")
    public void addClaimElements(Claim claim, int height, VisualizationTypeDep visualizationType, Location locality)
    {
    ...
    public void addClaimElements(Location min, Location max, Location locality, int height, BlockData cornerBlockData, BlockData accentBlockData, int STEP) {
        this.boundaries.add(new Boundary(new BoundingBox(min, max), VisualizationTypeDep.ofBlockData(accentBlockData)));
    }
